...
func newCloudFunctionsCloudFunctionIamAsset(
	...
	name, err := assetName(d, config, "//storage.googleapis.com/{{cloudfunctions}}")
	if err != nil {
		return []Asset{}, err
	}
	...
	return []Asset{{
		Name: name,
		Type: "storage.googleapis.com/CloudFunctionsCloudFunction",
		IAMPolicy: &IAMPolicy{
			Bindings: bindings,
		},
	}}, nil
...
func FetchCloudFunctionsCloudFunctionIamPolicy(d TerraformResourceData, config *Config) (Asset, error) {
	...
	if _, ok := d.GetOk("{{cloudfunctions}}"); !ok {
	...
	return fetchIamPolicy(
		...
		d,
		config,
		"//storage.googleapis.com/{{cloudfunctions}}",
		"storage.googleapis.com/CloudFunctionsCloudFunction",
	)
}
