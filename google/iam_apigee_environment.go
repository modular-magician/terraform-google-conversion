...
func ApigeeEnvironmentIamUpdaterProducer(d TerraformResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	m, err := getImportIdQualifiers([]string{"(?P<org_id>[^/]+)/environments/(?P<env_id>[^/]+)", "(?P<org_id>[^/]+)/(?P<env_id>[^/]+)", "(?P<env_id>[^/]+)"}, d, config, d.Get("env_id").(string))
	if err != nil {
		return nil, err
	}
...
func ApigeeEnvironmentIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"(?P<org_id>[^/]+)/environments/(?P<env_id>[^/]+)", "(?P<org_id>[^/]+)/(?P<env_id>[^/]+)", "(?P<env_id>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
