...
func newSecretManagerSecretIamAsset(
	...
	name, err := assetName(d, config, "//storage.googleapis.com/{{secretmanager}}")
	if err != nil {
		return []Asset{}, err
	}
	...
	return []Asset{{
		Name: name,
		Type: "storage.googleapis.com/SecretManagerSecret",
		IAMPolicy: &IAMPolicy{
			Bindings: bindings,
		},
	}}, nil
...
func FetchSecretManagerSecretIamPolicy(d TerraformResourceData, config *Config) (Asset, error) {
	...
	if _, ok := d.GetOk("{{secretmanager}}"); !ok {
	...
	return fetchIamPolicy(
		...
		d,
		config,
		"//storage.googleapis.com/{{secretmanager}}",
		"storage.googleapis.com/SecretManagerSecret",
	)
}
