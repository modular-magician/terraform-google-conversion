...
func stripNilScheduling(scheduling map[string]interface{}) map[string]interface{} {
	if scheduling == nil {
		return nil
	}
	for _, k := range []string{"localSsdRecoveryTimeout", "maxRunDuration", "onInstanceStopAction", "gracefulShutdown", "preemptionNoticeDuration"} {
		if v, ok := scheduling[k]; ok && isNilSchedulingValue(v) {
			delete(scheduling, k)
		}
	}
