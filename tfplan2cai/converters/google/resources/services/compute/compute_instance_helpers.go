...
func expandScheduling_v2(v interface{}) (map[string]interface{}, error) {
	...
	if v, ok := original["availability_domain"]; ok && v != nil {
		scheduling["availabilityDomain"] = v.(int)
	}
	if v, ok := original["max_run_duration"]; ok {
		transformedMaxRunDuration, err := expandComputeMaxRunDuration_v2(v)
		if err != nil {
			return nil, err
		}
	...
	if v, ok := original["on_instance_stop_action"]; ok {
		transformedOnInstanceStopAction, err := expandComputeOnInstanceStopAction_v2(v)
		if err != nil {
			return nil, err
		}
	...
	if v, ok := original["host_error_timeout_seconds"]; ok {
		scheduling["hostErrorTimeoutSeconds"] = v.(int)
	}
	if v, ok := original["maintenance_interval"]; ok {
		scheduling["maintenanceInterval"] = v.(string)
	}
	if v, ok := original["graceful_shutdown"]; ok {
		transformedGracefulShutdown, err := expandGracefulShutdown_v2(v)
		if err != nil {
			return nil, err
		}
	...
	if v, ok := original["preemption_notice_duration"]; ok {
		transformedPreemptionNoticeDuration, err := expandComputePreemptionNoticeDuration_v2(v)
		if err != nil {
			return nil, err
		}
	...
	if v, ok := original["local_ssd_recovery_timeout"]; ok {
		transformedLocalSsdRecoveryTimeout, err := expandComputeLocalSsdRecoveryTimeout_v2(v)
		if err != nil {
			return nil, err
		}
...
func expandNetworkInterfaces_v2(d tpgresource.TerraformResourceData, config *transport_tpg.Config) ([]interface{}, error) {
	...
	for i, raw := range configs {
		...
		iface := map[string]interface{}{
			...
			"nicType":                  data["nic_type"].(string),
			"stackType":                data["stack_type"].(string),
			"queueCount":               data["queue_count"].(int),
			"ipv6AccessConfigs":        expandIpv6AccessConfigs_v2(data["ipv6_access_config"].([]interface{})),
			"ipv6Address":              data["ipv6_address"].(string),
			"internalIpv6PrefixLength": data["internal_ipv6_prefix_length"].(int),
			"igmpQuery":                data["igmp_query"].(string),
		}
		if macAddressObj, ok := data["mac_address"]; ok && macAddressObj.(string) != "" {
