...
func GetGkeonpremVmwareAdminClusterApiObject(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (map[string]interface{}, error) {
	...
	preparedSecretsProp, err := expandGkeonpremVmwareAdminClusterPreparedSecrets(d.Get("prepared_secrets"), d, config)
	if err != nil {
		return nil, err
	} else if v, ok := d.GetOkExists("prepared_secrets"); !tpgresource.IsEmptyValue(reflect.ValueOf(preparedSecretsProp)) && (ok || !reflect.DeepEqual(v, preparedSecretsProp)) {
		obj["preparedSecrets"] = preparedSecretsProp
	}
...
func expandGkeonpremVmwareAdminClusterPreparedSecrets(v interface{}, d tpgresource.TerraformResourceData, config *transport_tpg.Config) (interface{}, error) {
	l := v.([]interface{})
	if len(l) == 0 || l[0] == nil {
		return nil, nil
	}
	...
	transformedEnabled, err := expandGkeonpremVmwareAdminClusterPreparedSecretsEnabled(original["enabled"], d, config)
	if err != nil {
		return nil, err
	} else if val := reflect.ValueOf(transformedEnabled); val.IsValid() && !tpgresource.IsEmptyValue(val) {
		transformed["enabled"] = transformedEnabled
...
func expandGkeonpremVmwareAdminClusterPreparedSecretsEnabled(v interface{}, d tpgresource.TerraformResourceData, config *transport_tpg.Config) (interface{}, error) {
	return v, nil
}
