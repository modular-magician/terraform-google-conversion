...
func SecretManagerRegionalRegionalSecretIamUpdaterProducer(d tpgresource.TerraformResourceData, config *transport_tpg.Config) (tpgiamresource.ResourceIamUpdater, error) {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<region>[^/]+)/secrets/(?P<secret_id>[^/]+)", "(?P<project>[^/]+)/(?P<region>[^/]+)/(?P<secret_id>[^/]+)", "(?P<region>[^/]+)/(?P<secret_id>[^/]+)", "(?P<secret_id>[^/]+)"}, d, config, d.Get("secret_id").(string))
	if err != nil {
		return nil, err
	}
...
func SecretManagerRegionalRegionalSecretIdParseFunc(d *schema.ResourceData, config *transport_tpg.Config) error {
	...
	m, err := tpgresource.GetImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<region>[^/]+)/secrets/(?P<secret_id>[^/]+)", "(?P<project>[^/]+)/(?P<region>[^/]+)/(?P<secret_id>[^/]+)", "(?P<region>[^/]+)/(?P<secret_id>[^/]+)", "(?P<secret_id>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *SecretManagerRegionalRegionalSecretIamUpdater) qualifyRegionalSecretUrl(methodIdentifier string) (string, error) {
	urlTemplate := fmt.Sprintf("{{SecretManagerRegionalBasePath}}%s:%s", fmt.Sprintf("projects/%s/locations/%s/secrets/%s", u.project, u.region, u.secretId), methodIdentifier)
...
func (u *SecretManagerRegionalRegionalSecretIamUpdater) GetResourceId() string {
	return fmt.Sprintf("projects/%s/locations/%s/secrets/%s", u.project, u.region, u.secretId)
}
