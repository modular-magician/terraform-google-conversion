...
const NetworkServicesEdgeCacheKeysetAssetNameRegex string = "projects/(?P<project>[^/]+)/locations/global/edgeCacheKeysets"

type NetworkServicesEdgeCacheKeysetConverter struct {
	name   string
	schema map[string]*schema.Schema
}

func NewNetworkServicesEdgeCacheKeysetConverter(name string, schema map[string]*schema.Schema) common.Converter {
	return &NetworkServicesEdgeCacheKeysetConverter{
		name:   name,
		schema: schema,
	}
}

func (c *NetworkServicesEdgeCacheKeysetConverter) Convert(assets []*caiasset.Asset) ([]*common.HCLResourceBlock, error) {
...
func (c *NetworkServicesEdgeCacheKeysetConverter) convertResourceData(asset *caiasset.Asset, config *transport_tpg.Config) (*common.HCLResourceBlock, error) {
	...
	hcl, _ := resourceNetworkServicesEdgeCacheKeysetRead(assetResourceData, config)
...
func resourceNetworkServicesEdgeCacheKeysetRead(resource map[string]interface{}, config *transport_tpg.Config) (map[string]interface{}, error) {
	result := make(map[string]interface{})
	var resource_data *schema.ResourceData = nil

	result["description"] = flattenNetworkServicesEdgeCacheKeysetDescription(resource["description"], resource_data, config)
	result["labels"] = flattenNetworkServicesEdgeCacheKeysetLabels(resource["labels"], resource_data, config)
	result["public_key"] = flattenNetworkServicesEdgeCacheKeysetPublicKey(resource["publicKeys"], resource_data, config)
	result["validation_shared_keys"] = flattenNetworkServicesEdgeCacheKeysetValidationSharedKeys(resource["validationSharedKeys"], resource_data, config)
	result["terraform_labels"] = flattenNetworkServicesEdgeCacheKeysetTerraformLabels(resource["labels"], resource_data, config)
	result["effective_labels"] = flattenNetworkServicesEdgeCacheKeysetEffectiveLabels(resource["labels"], resource_data, config)
